...
def test():
    testeql(groupingDishes([["Salad","Tomato","Cucumber","Salad","Sauce"], 
                            ["Pizza","Tomato","Sausage","Sauce","Dough"], 
                            ["Quesadilla","Chicken","Cheese","Sauce"], 
                            ["Sandwich","Salad","Bread","Tomato","Cheese"]]),
...
[["Cheese","Quesadilla","Sandwich"], 
 ...
 ["Tomato","Pizza","Salad","Sandwich"]])
    ...
    testeql(groupingDishes([["Pasta","Tomato Sauce","Onions","Garlic"], 
 ["Chicken Curry","Chicken","Curry Sauce"], 
...
def groupingDishes(dishes):
    ...
    for d in dishes:
        dish = d[0]
        for ingr in d[1:]:
            byIngredient[ingr].append(dish)
    out = []

    for ingr in sorted(byIngredient.keys()):
        if len(byIngredient[ingr]) > 1:
            out.append([ingr] + sorted(byIngredient[ingr]))
    return out
