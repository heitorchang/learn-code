...
(defun deque-range (d lst)
  (if (= 0 (length lst))
      d
      (progn
        (push-back d (car lst))
        (deque-range d (cdr lst)))))
...
(defun dr (s)
  (deque-range (make-instance 'deque) (soRange 1 s)))

(defun rot (d)
  (if (<= (element-count d) 2)
      d
      (progn 
        (push-back d (pop-front d))
        (push-back d (pop-front d))
        d)))


(defun iter (d a)
  (if (= 0 (element-count d))
      a
      (progn
        (rot d)
        (let ((last (pop-back d)))
...
(defun pjCircle (s w)
  (subseq (reverse (iter (dr s) '())) (- s w) s))
