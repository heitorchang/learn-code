...
var BiwaScheme = (function () {
  ...
  const GitCommit = "6cb6a0cc9da5f2a40d4afe10d70713d73d44f3bd";
  ...
  function ie10IsDataView(obj) {
    return (
      obj != null && isFunction$1$1(obj.getInt8) && isArrayBuffer(obj.buffer)
    );
  }
  ...
  function collectNonEnumProps(obj, keys) {
    ...
    var proto =
      (isFunction$1$1(constructor) && constructor.prototype) || ObjProto;
  ...
  function isEmpty(obj) {
    ...
    if (
      ...
      (isArray(obj) || isString$1(obj) || isArguments$1(obj))
    )
      return length === 0;
  ...
  function deepEq(a, b, aStack, bStack) {
    ...
    if (!areArrays) {
      ...
      if (
        aCtor !== bCtor &&
        !(
          isFunction$1$1(aCtor) &&
          aCtor instanceof aCtor &&
          isFunction$1$1(bCtor) &&
    ...
    if (areArrays) {
      ...
      while (length--) {
        ...
        key = _keys[length];
        if (!(has$1(b, key) && eq$1(a[key], b[key], aStack, bStack)))
          return false;
      }
    }
  ...
  function ie11fingerprint(methods) {
    ...
    return function (obj) {
      ...
      for (var i = 0; i < length; i++) {
        if (!isFunction$1$1(obj[methods[i]])) return false;
      }
      // If we are testing against `WeakMap`, we need to ensure that
      // `obj` doesn't have a `forEach` method in order to distinguish
      // it from a regular `Map`.
      return methods !== weakMapMethods || !isFunction$1$1(obj[forEachName]);
    };
  }
  ...
  function functions(obj) {
    var names = [];
    for (var key in obj) {
      if (isFunction$1$1(obj[key])) names.push(key);
    }
    return names.sort();
  }
  ...
  function result(obj, path, fallback) {
    ...
    var length = path.length;
    if (!length) {
      return isFunction$1$1(fallback) ? fallback.call(obj) : fallback;
    }
    for (var i = 0; i < length; i++) {
  ...
  class Hashtable {
    ...
    _apply_pair(func) {
      var a = [];
      values(this.pairs_of).forEach(function (pairs) {
  ...
  var macroexpand_1 = function (x) {
    if (x instanceof Pair) {
      ...
      if (x.car instanceof BiwaSymbol && TopEnv[x.car.name] instanceof Syntax) {
        ...
        throw new Error(
          "macroexpand-1: `" + to_write$1(x) + "' is not a macro"
        );
    }
    return x;
  };
  ...
  define_libfunc("truncate/", 2, 2, function (ar) {
    assert_number(ar[0]);
    assert_number(ar[1]);
    return new Values$1([truncate_q(ar[0], ar[1]), truncate_r(ar[0], ar[1])]);
  });
  ...
  define_syntax("delay", function (x) {
    ...
    if (x.cdr.cdr !== nil) {
      throw new BiwaError(
        "malformed delay: too many arguments: " + write_ss(x)
      );
    }
    var expr = x.cdr.car;
  ...
  class Dumper {
    ...
    add_fold(s) {
      ...
      if (lines.length > FOLD_LIMIT) {
        ...
        this.n_folds++;
        return [
          lines.slice(0, FOLD_LIMIT).join("<br>"),
          fold_btn,
          fold_start,
          lines.slice(FOLD_LIMIT).join("<br>"),
          fold_end,
        ].join("");
      } else {
        return s;
    ...
    dump_stack(stk, size) {
      ...
      for (var i = size - 1; i >= 0; i--) {
        s +=
          ...
          "]</td>" +
          "<td>" +
          truncate(this.dump_obj(stk[i]), STACK_MAX_LEN) +
          "</td></tr>";
      }
      return s + "</table>";
    }
