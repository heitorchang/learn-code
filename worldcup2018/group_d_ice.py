...
def simulate(optimistic=False):
    """Assume optimistically that in every tie (between 2nd and 3rd places) Iceland comes ahead (it is second to arg in this group"""

    arg = Team("ARG")
    ice = Team("ICE", favorite=1)
    cro = Team("CRO")
    nig = Team("NIG")
...
""")
    ...
    for result in results:
        ...
        match(ice_h, cro_h, result[0])
        match(nig_h, arg_h, result[1])
        
        standings_favorite = sorted([arg_h, ice_h, cro_h, nig_h], key=attrgetter('favorite'), reverse=optimistic)
        ...
        if standings_pts.index(ice_h) < 2:
            print("*ADVANCE*", end=" ")
            ice_in += 1
        else:
            print("_go home_", end=" ")
    ...
    print("'ICE in' in a total of", ice_in, "out of", 3**2, "scenarios")
